...
- `description` is required, non-empty, and sent **exactly as supplied**, including ordinary line breaks. Empty descriptions have not been confirmed acceptable. No descriptions are generated or borrowed.
...
4. Allocate a **new** photo with `POST /api/v1/listing/{id}/photo`. PUT the local bytes to the issued HTTPS S3 URL with image Content-Type and **without Gameflip authorization**. Redirects and unexpected storage hosts are blocked.
5. PATCH this draft's own photo to active, display order 0, and cover photo. GET and verify all fields and the active cover photo. PATCH to `onsale` only after verification, then GET to confirm. PATCH requests use `If-Match` version protection and a draft-status test to reject concurrent changes.
...
- Rerun the **same input and images** with live flags to resume that exact draft. A changed payload/image, changed remote listing, or another matching listing requires review. Recovery never edits pre-existing listings merely because their title matches.
