...
5. GET and verify all fields plus the active cover photo **if an image was supplied**. PATCH to `onsale` only after verification, then GET to confirm. PATCH requests use `If-Match` version protection and a draft-status test to reject concurrent changes. Omitted images do not bypass field, status, ownership, version or recovery checks.
