...
export const HELP_TOPICS = [
  ...
  { name: 'Business expenses', value: 'expense' },
...
export function buildHelpPages(topic = 'overview', itemNames = []) {
  ...
  const descriptions = {
    overview: [
      ...
      '**OPERATING EXPENSES**',
      '`RAR EXPENSE`, `MR EXPENSE`, or `BUSINESS EXPENSE` for shared overhead.',
      '`PURCHASE` adds inventory acquisition cost. `EXPENSE` records overhead and never changes stock.',
      '**FARM — RAR only**',
      ...
      'Shows **Actual Wallet Credit**, **Platform Tax**, **Item Purchase Spending**, **Operating Expenses**, and **Net Profit** separately for USD / MYR / PHP / IDR.',
    ...
    expense: [
      ...
      '```text\nMR EXPENSE - SOFTWARE\n20 USD\n```',
      '```text\nBUSINESS EXPENSE - DOMAIN\n60 MYR\n```',
      'Use `BUSINESS EXPENSE` for shared overhead. Post it in either operations channel.',
      'An optional third line is a free-text category, for example `Cloud / VPS`; omitted categories become `Other`.',
      '**PURCHASE = inventory acquisition. EXPENSE = operating/business overhead.**',
      'Descriptions are free text and are never resolved against the item catalog.',
    ].join('\n'),
    farm: [
      '**Farm claim**',
      '```text\nRAR FARM - 1 CYCLE\n```',
    ...
    monthly: [
      ...
      '',
      '**Net Profit = Actual Wallet Credit − Item Purchase Spending − Operating Expenses**',
