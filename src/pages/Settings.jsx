...
export default function Settings({ data, refresh, notify }) {
  ...
  const [newItem, setNewItem] = useState({ name: '', category: 'General', aliases: '', gem_value_min: '', gem_value_max: '', is_farm_item: false })
  ...
  const filteredItems = useMemo(() => data.items.filter((item) => item.kind === 'item' && `${item.name} ${item.category || ''}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => a.name.localeCompare(b.name)), [data.items, search])
  ...
  const saveItem = async (item) => {
    ...
    try {
      const payload = {
        ...
        p_aliases: aliasesFromText(draft.aliases),
        p_gem_value_min: min,
        p_gem_value_max: max ?? min,
        p_active: Boolean(draft.active),
        p_is_farm_item: Boolean(draft.is_farm_item),
      }
      const { data: savedId, error } = await supabase.rpc('rar_upsert_catalog_item', payload)
      if (error) throw error
      const { data: saved, error: verifyError } = await supabase.from('rar_items').select('*').eq('id', savedId).single()
      if (verifyError || !saved || saved.name !== payload.p_name || saved.active !== payload.p_active || saved.is_farm_item !== payload.p_is_farm_item) throw new Error('The saved item settings could not be verified.')
      await refresh()
  ...
  const addItem = async (event) => {
    ...
    try {
      const { data: savedId, error } = await supabase.rpc('rar_upsert_catalog_item', {
        p_item_id: null,
        p_name: name,
        p_category: newItem.category.trim() || 'General',
        p_aliases: aliasesFromText(newItem.aliases),
        p_gem_value_min: min,
        p_gem_value_max: max ?? min,
        p_is_farm_item: newItem.is_farm_item,
        p_active: true,
      })
      if (error) throw error
      const { data: saved, error: verifyError } = await supabase.from('rar_items').select('*').eq('id', savedId).single()
      ...
      await refresh()
      setAddOpen(false)
      setNewItem({ name: '', category: 'General', aliases: '', gem_value_min: '', gem_value_max: '', is_farm_item: false })
  ...
  return (
    <div className="page-stack">
      <PageHeader eyebrow="RAR · Prices / Setup" title="Catalog Manager" description="Maintain safe item metadata and fee guides without changing stock or historical IDs." action={<Button onClick={() => setAddOpen(true)}><Plus size={18} />Add RAR item</Button>} />
            ...
            {filteredItems.map((item) => {
              ...
              return (
                <article className={`settings-item ${!draft.active ? 'is-disabled' : ''}`} key={item.id}>
                  ...
                  <div className="settings-item__name"><label><span>Name</span><input value={draft.name} onChange={(event) => setItemDrafts((current) => ({ ...current, [item.id]: { ...current[item.id], name: event.target.value } }))} /></label><span>{formatQuantity(item.stock)} tracked · ID preserved</span><label><span>Category</span><input value={draft.category} onChange={(event) => setItemDrafts((current) => ({ ...current, [item.id]: { ...current[item.id], category: event.target.value } }))} /></label><label><span>Aliases</span><input value={draft.aliases} placeholder="Comma-separated" onChange={(event) => setItemDrafts((current) => ({ ...current, [item.id]: { ...current[item.id], aliases: event.target.value } }))} /></label></div>
      ...
      <Dialog open={addOpen} title="Add a new item" description="New items start with zero stock. Use Purchases, Farming, or Stocktake to add units." onClose={() => !savingKey && setAddOpen(false)}>
        <form className="form-stack" onSubmit={addItem}>
          ...
          <div className="form-grid form-grid--2"><Field label="Category"><input value={newItem.category} onChange={(event) => setNewItem((current) => ({ ...current, category: event.target.value }))} placeholder="General" /></Field><Field label="Aliases" hint="Comma-separated"><input value={newItem.aliases} onChange={(event) => setNewItem((current) => ({ ...current, aliases: event.target.value }))} /></Field></div>
